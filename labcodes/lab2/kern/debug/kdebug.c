...
print_stackframe(void) {
	...
	for (i = 0; ebp != 0 && i < STACKFRAME_DEPTH; ++i) {
		...
		cprintf("\n");
		print_debuginfo(eip - 1);    // Prints the calling function info.
		eip = *(uint32_t*)(ebp + 4); // Assigns the calling function's return addr to eip.
		ebp = *(uint32_t*)ebp;       // Gets Caller's ebp.
	}
}
